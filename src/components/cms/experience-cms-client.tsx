...
import { Plus, Trash2, Edit2, Briefcase, Building, Calendar, Wrench, Save, Loader2 } from "lucide-react";
import { Dialog, DialogContent, DialogHeader, DialogTitle, DialogFooter } from "@/components/ui/dialog";

interface ExperienceCmsClientProps {
  initialItems: any[];
}

export function ExperienceCmsClient({ initialItems }: ExperienceCmsClientProps) {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      if (editingItem) {
        const updated = await updateExperience(editingItem.id, {
          position,
          positionEn,
          company,
          companyEn,
        ...
        toast.success("Pengalaman karir berhasil diperbarui!");
      } else {
        ...
        toast.success("Pengalaman karir baru berhasil ditambahkan!");
      }
      setIsAddOpen(false);
      resetForm();
    } catch (err) {
      console.error(err);
      toast.error("Gagal menyimpan pengalaman karir");
    } finally {
      setIsSaving(false);
    }
  };
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      toast.success("Pengalaman karir berhasil dihapus!");
      setDeletingId(null);
    } catch (err) {
      console.error(err);
      toast.error("Gagal menghapus pengalaman karir");
    } finally {
      setIsSaving(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center pb-4 border-b border-slate-200 dark:border-zinc-800">
        <div>
          <h3 className="text-xl font-black text-slate-900 dark:text-white">Daftar Pengalaman Karir ({items.length})</h3>
          <p className="text-xs text-slate-500 dark:text-zinc-400">Kelola riwayat pekerjaan QA dan magang profesional Anda.</p>
        </div>
        <Button onClick={handleOpenAdd} className="bg-gradient-to-r from-blue-600 to-cyan-500 hover:from-blue-500 hover:to-cyan-400 text-white font-bold rounded-xl shadow-md">
          <Plus className="w-4 h-4 mr-2" /> Tambah Karir
        </Button>
      </div>
      ...
      <div className="space-y-4">
        {items.length === 0 ? (
          <div className="p-12 text-center border-2 border-dashed border-slate-200 dark:border-zinc-800 rounded-3xl">
            ...
            <p className="text-sm font-bold text-slate-600 dark:text-zinc-400">Belum ada data pengalaman karir.</p>
          </div>
        ) : (
          items.map((item) => (
            <div key={item.id} className="p-6 rounded-2xl bg-white dark:bg-[#070e20] border border-slate-200 dark:border-cyan-500/20 shadow-sm flex flex-col md:flex-row justify-between items-start md:items-center gap-4 hover:border-cyan-500/50 transition-all">
              <div className="space-y-1.5 flex-1">
                <div className="flex items-center gap-2">
                  <h4 className="text-lg font-extrabold text-slate-900 dark:text-white">{item.position}</h4>
                  <span className="text-xs px-2.5 py-0.5 rounded-full bg-cyan-500/10 text-cyan-400 font-bold border border-cyan-500/20">{item.company}</span>
                </div>
                ...
                {item.technologies && (
                  <p className="text-[11px] text-slate-400 dark:text-zinc-400 pt-1 font-mono">
                    Tools/Tech: <span className="text-slate-700 dark:text-zinc-300 font-medium">{item.technologies}</span>
                  </p>
                )}
              </div>
        ...
        <DialogContent className="sm:max-w-[600px] max-h-[90vh] overflow-y-auto bg-white dark:bg-[#070e20] border-slate-200 dark:border-cyan-500/30 rounded-3xl p-6">
          <DialogHeader>
            <DialogTitle className="text-xl font-bold text-slate-900 dark:text-white">
              {editingItem ? "Edit Pengalaman Karir" : "Tambah Pengalaman Karir"}
            </DialogTitle>
          </DialogHeader>
          ...
          <form onSubmit={handleSave} className="space-y-4 py-2">
            ...
            <div className="space-y-2">
              <Label className="text-xs font-bold uppercase">Tools & Metodologi (pisahkan dengan koma)</Label>
              <Input value={technologies} onChange={(e) => setTechnologies(e.target.value)} placeholder="Regression Testing, Smoke Testing, UAT, Manual Testing" className="rounded-xl bg-slate-50 dark:bg-zinc-950" />
            </div>
      ...
      <Dialog open={!!deletingId} onOpenChange={() => setDeletingId(null)}>
        <DialogContent className="sm:max-w-[400px] bg-white dark:bg-[#070e20] border-slate-200 dark:border-cyan-500/30 rounded-3xl p-6">
          <DialogHeader>
            <DialogTitle className="text-xl font-bold text-red-600">Hapus Pengalaman Karir</DialogTitle>
          </DialogHeader>
          <p className="text-sm text-slate-600 dark:text-zinc-300 py-2">Apakah Anda yakin ingin menghapus data pengalaman karir ini?</p>
